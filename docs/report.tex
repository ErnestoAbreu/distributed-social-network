...
Con respecto a la seguridad en la comunicación, gRPC que proporciona una comunicación segura clientes y servidores mediante el uso de
canales (TLS/SSL), protegiendo la información de posibles ataques como la intercepción de datos.

El uso del anillo de Chord garantiza una distribución descentralizada de datos, reduciendo puntos de falla, sin embargo la seguridad depende
de la validación de nodos participantes para evitar que la entrada de nodos maliciosos comprometan el sistema. La separación de las redes de Docker permite aislar el tráfico entre las redes de clientes y servidores, esto minimiza la exposición de
componentes críticos.

El sistema utilizará autenticación de usuarios para garantizar que solo personas autorizadas puedan utilizar los servicios que ofrece el sistema, como son
la publicación de posts, seguir nuevos usuarios; además se hará uso de tokens seguros como JWT para fortalecer esta seguridad. En cuanto a la autorización, los
servidores verificarán los permisos de los usuarios para evitar que usuarios no autorizados realicen operaciones sensibles/no permitidas en el sistema, reduciendo el
riesgo a accesos indebidos.
